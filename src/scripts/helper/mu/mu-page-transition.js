(function(global) {
    ...
    var isAnimating = false,
        ...
        endCurrPage = false,
        endNextPage = false,
        animationEnd = mu.util.animationEnd().animationend;
    /**
     * @namespace
     * @global
     * @ignore
    ...
    var _core = {
         ...
         * 初始化转场数据
         * @param  {object} opts
         * @example var page = mu.widget.pagetrans;
         * page.init({
        ...
        _initClass: function() {
            $(options.target).each(function() {
                ...
                $this.addClass('page' + index)
                    .attr({
                        'data-original-class': options.target.replace('.','')+' page' + index,
                        'id':'page' + index
                    });
        ...
        destroy: function(){
            $(options.target).each(function(){
                var $this = $(this),
                    index = $this.index();
                $this.removeAttr('data-original-class').removeAttr('id').removeClass('page'+index);
            });
        },
        /**
        ...
        _changePage: function($oupage, $inpage, ou_class, in_class) {
            ...
            mu.widget.pagetrans.isAnimating = isAnimating = true;
        ...
        _afterAnimation: function($oupage, $inpage) {
            endCurrPage = false;
            endNextPage = false;
            mu.widget.pagetrans.isAnimating = isAnimating = false;
            $oupage.attr('class', $oupage.attr('data-original-class'));
            $inpage.attr('class', $inpage.attr('data-original-class') + ' page-current');

            mu.widget.pagetrans.current = current = $inpage.index();
